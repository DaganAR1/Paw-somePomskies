...
import { 
  ...
  INITIAL_PARENTS,
  INITIAL_SCHEDULE,
  INITIAL_GUARDIAN_DOGS,
  SOCIAL_LINKS, 
  BREEDER_CONTACT_EMAIL,
  BREEDER_PHONE
} from './constants';
import { Puppy, BlogPost, Parent, ScheduleEvent, GuardianDog } from './types';
...
const App: React.FC = () => {
  ...
  // -- FAVICON SYNC --
  useEffect(() => {
    if (siteAssets.branding.favicon) {
      ...
      link.href = siteAssets.branding.favicon;
      link.type = 'image/png';
    }
  }, [siteAssets.branding.favicon]);
  ...
  useEffect(() => {
    ...
    const checkSession = async () => {
      ...
      if (session) {
        ...
        // Fallback to legacy session check if needed, but Supabase Auth is primary
    ...
    const initData = async () => {
      try {
        const [p, par, g, s, b, a] = await Promise.all([
          dataService.getPuppies(),
          dataService.getParents(),
          dataService.getGuardianDogs(),
        ...
        setSchedule(s.length > 0 ? s : INITIAL_SCHEDULE);
...
}, [puppies, isDataSyncEnabled, isAdmin]);
...
}, [parents, isDataSyncEnabled, isAdmin]);

  useEffect(() => { 
    if (isDataSyncEnabled && isAdmin) {
      setSyncStatus('saving');
      dataService.saveGuardianDogs(guardianDogs)
        .then(() => setSyncStatus('idle'))
        .catch(err => {
          console.error(err);
          setSyncStatus('error');
          setLastError(err.message || 'Failed to save guardian dogs');
        });
      safeSaveLocal('pawsome_guardian', guardianDogs); 
    }
  }, [guardianDogs, isDataSyncEnabled, isAdmin]);
...
}, [schedule, isDataSyncEnabled, isAdmin]);
...
}, [blogPosts, isDataSyncEnabled, isAdmin]);
...
}, [siteAssets, isDataSyncEnabled, isAdmin]);
  ...
  return (
    <div className="min-h-screen relative flex flex-col">
      <Navbar 
        ...
        onNavigateGuardian={() => navigateTo('guardian')}
      />
      ...
      <div className="flex-grow">
        ...
        {currentView === 'guardian' && (
          <GuardianPage 
            ...
            heroImage={siteAssets.sections.guardianHero} 
            programImage={siteAssets.sections.guardianProgram}
          />
        )}
        ...
        {currentView === 'admin' && (
          <AdminDashboard 
            ...
            guardianDogs={guardianDogs} setGuardianDogs={setGuardianDogs}
            ...
            lastError={lastError || (!isDataSyncEnabled ? 'Supabase connection not established. Changes will not be saved to cloud.' : null)}
          />
        )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-12 text-center md:text-left">
            ...
            <div>
              ...
              <ul className="space-y-4 opacity-70 text-sm">
                ...
                <li><button onClick={() => navigateTo('guardian')} className="hover:text-teal-400 transition-colors">Guardian Homes</button></li>
              </ul>
            </div>
      ...
      {/* Floating admin portal badge — only visible when logged in, hidden on admin view */}
      {isAdmin && currentView !== 'admin' && (
        <button
          onClick={() => navigateTo('admin')}
          className="fixed bottom-6 right-6 z-50 flex items-center gap-2.5 px-4 py-3 bg-teal-700 hover:bg-teal-600 text-white rounded-2xl shadow-2xl shadow-teal-900/60 transition-all hover:-translate-y-0.5 active:scale-95"
          title="Go to Breeder Portal"
        >
          ...
          <span className="text-[11px] font-black uppercase tracking-widest">Breeder Portal</span>
        </button>
      )}
    </div>
  );
