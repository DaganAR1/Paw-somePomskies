...
import { Puppy, Parent, ScheduleEvent, BlogPost, GuardianDog } from '../types';
...
// 🚨 SAFETY CHECK — prevent wiping the whole table
if (puppies.length === 0 && existingIds.length > 0) {
  console.warn("Prevented full puppies table wipe.");
  return;
}
...
// 🚨 Prevent wiping entire table
if (parents.length === 0 && existingIds.length > 0) {
  console.warn("Blocked accidental delete of all parents");
  return;
}
...
const newIds = parents.map(p => p.id);
  ...
  async saveSchedule(schedule: ScheduleEvent[]) {
    ...
    const existingIds = (existing || []).map(item => item.id);
    const newIds = schedule.map(s => s.id);
    ...
    if (schedule.length > 0) {
      const cleanSchedule = schedule.map(s => ({
        id: s.id || `s${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
        event: s.event,
        period: s.period,
        date: s.date,
        details: s.details
      ...
      const { error } = await supabase.from('schedule').upsert(cleanSchedule);
      if (error) throw error;
    }
  },
...
if (posts.length === 0 && existingIds.length > 0) {
  console.warn("Blocked accidental delete of all blog posts");
  return;
}
...
const newIds = posts.map(p => p.id);
  ...
  // Guardian Dogs
  async getGuardianDogs(): Promise<GuardianDog[]> {
    const { data, error } = await supabase.from('guardian_dogs').select('*');
    if (error) throw error;
    return data || [];
  },
  async saveGuardianDogs(dogs: GuardianDog[]) {
    const { data: existing } = await supabase.from('guardian_dogs').select('id');
    const existingIds = (existing || []).map(item => item.id);

    if (dogs.length === 0 && existingIds.length > 0) {
      console.warn("Blocked accidental delete of all guardian dogs");
      return;
    }

    const newIds = dogs.map(d => d.id);
    const toDelete = existingIds.filter(id => !newIds.includes(id));
    if (toDelete.length > 0) {
      await supabase.from('guardian_dogs').delete().in('id', toDelete);
    }

    if (dogs.length > 0) {
      const cleanDogs = dogs.map(d => ({
        id: d.id,
        name: d.name,
        gender: d.gender,
        age: d.age,
      ...
      const { error: upsertError } = await supabase.from('guardian_dogs').upsert(cleanDogs);
      if (upsertError) throw upsertError;
    }
  }
};
